...
網站發布不包含在預設流程內。即使 `build` 與 `validate` 已完成，也只有在使用者另外明確要求發布時，才能依 `workflow/GITHUB_PUBLISHING.md` 獨立執行 GitHub 發布。
...
回報 operation、變更檔案、Script 檢查結果、待人工確認項目，以及是否另行執行網站發布。Google Docs 預設不修改；只有使用者明確要求例外且已執行時，才另外回報 Google Docs 變更。未完成瀏覽器與列印 QA 時，不得描述為可正式發布。
