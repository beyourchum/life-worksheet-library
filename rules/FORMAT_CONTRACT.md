...
矩陣標記內只放 `- 列題目` 與空白行；說明、標題、分頁及其他填答標記放在矩陣外。lint 遇到矩陣內無法呈現的內容須回報行號並停止建置，不得默默略過。調整位置時保留原文與題意；位置不明時先確認。
...
小提醒、操作提示或 AI 輔助說明統一使用單行引用文字，套用帶箭頭與上下分隔線的提示框樣式：
...
提示框的名稱與說明放在同一行；不要另用 H2 或 H3 製作提示框標題。

提示框預設不顯示箭頭。小提醒、最後鼓勵的話與 AI 協助說明需要箭頭時，在引用文字前加入獨立一行 `<!-- with-arrow -->`。
